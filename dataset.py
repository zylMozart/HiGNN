...
class NCDataset(object):
    ...
    def get_idx_split(self, split_type='random', train_prop=.5, valid_prop=.25, run=0):
        """
        ...
        if split_type == 'random':
            ...
            splits_file_path = 'splits/' + self.name.lower() + \
                '_split_0.6_0.2_' + str(run) + '.npz'
            ...
            train_idx = list(np.where(train_mask == 1)[0])
            valid_idx = list(np.where(val_mask == 1)[0])
            test_idx = list(np.where(test_mask == 1)[0])
            split_idx = {'train': torch.LongTensor(train_idx),
                         'valid': torch.LongTensor(valid_idx),
                         'test': torch.LongTensor(test_idx)}
